...
def update_vm_vmfex_support(api, dc, src_cluster, dest_cluster, dest_net):
    ...
    vms = api.vms.list(query='cluster=' + src_cluster)
    # Create vNIC profiles corresponding to UCS profiles.
    create_vnics(api, dc, vms, network)

    # Upgrade the VMs.
    upgrade_vms(api, dc, vms, dest_cluster, network)
...
def upgrade_vms(api, dc, vms, dest_cluster, network):
    for vm in vms:
        vmname = vm.get_name()
        ...
        if vm.get_placement_policy().get_hosts():
            logging.info("VM: '%s' is pinned to host(s) and cannot be upgraded", vmname)
            continue
        ...
        try:
            if update_vnics_for_vm(api, dc, network, vm, vmfex):
                delete_vmfex_custom_property(vm)
                move_vm_to_cluster(vm, dest_cluster)
        except Exception as e:
            logging.exception("Failed to upgrade VMFEX support for VM: '%s'.", vmname)
            continue


def update_vnics_for_vm(api, dc, network, vm, vmfex):
        ...
        if vnic.mac.address in vmfex_dict:
            profile = next(
                (
                    ...
                    if p.get_network().get_id() == network.get_id()
                ),
                None
            )
            if not profile:
                create_vnic(api, dcname, profiles, dest_net)
                logging.error("Profile: '%s' is not defined in network: '%s'", vmfex_dict[vnic.mac.address], network.get_name())
                return False
...
def create_vnics(api, dc, vms, network):
    for vm in vms:
        logging.info("Processing UCS profiles used by VM: '%s'", vm.get_name())
        vmfex = get_vmfex(vm)
        if not vmfex:
            continue
        ...
        for mac, profile_name in vmfex_dict.items():
            ...
            if not profile:
                create_vnic(api, dc.get_name(), profile_name, network)
            else:
                logging.info("vNIC profile: '%s' already exists", profile_name)

def create_vnic(api, dcname, profile, network):
    logging.info("Creating vNIC Profile: '%s' in network: '%s'", profile, network.get_name())
    vnic_profile = params.VnicProfile(name=profile, network=network)
    cp = params.CustomProperties([params.CustomProperty(name='vmfex', value=profile)])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-src-cluster", "--srccluster", help="Cluster to upgrade VMs")
    parser.add_argument("-dest-cluster", "--destcluster", help="Cluster to move VMs to")
    ...
    try:
        update_vm_vmfex_support(api, args.datacenter, args.srccluster, args.destcluster, args.destnet)
    finally:
        api.disconnect()
